...
const {
  createAuctionState, placeBid, checkExpiry, autoFillRemaining, currentPlayer, minIncrement,
  pauseAuction: pauseLot, resumeAuction: resumeLot, skipCurrentLot,
} = require('./engine/auctionEngine');
...
async function resumeAuction(io) {
  ...
  if (state.status !== 'active' && state.status !== 'paused') { await kvDelete(KV_KEY); return; }
...
function publicAuctionState() {
  ...
  return {
    ...
    nextMinBid: state.currentBidTeamId ? state.currentBid + minIncrement(state.currentBid) : state.currentBid,
    deadline: state.deadline,
    pausedRemainingMs: state.status === 'paused' ? (state.pauseRemainingMs ?? 0) : null,
    queueRemaining: Math.max(0, state.queue.length - state.queuePosition),
...
async function startAuction(io, stageId = null, lotType = 'player') {
  if (current && (current.state.status === 'active' || current.state.status === 'paused')) {
...
/** Admin-triggered pause: freezes the current lot's countdown in place.
 * Bidding is rejected while paused; the clock resumes with whatever time
 * was left when unpauseAuction is called (survives a server restart too,
 * since resumeAuction() at boot reloads 'paused' state as-is). */
async function pauseAuction(io) {
  if (!current) throw new Error('No auction is currently active.');
  pauseLot(current.state);
  await persistCurrent();
  io.to('auction-room').emit('auction:state', publicAuctionState());
  return publicAuctionState();
}

async function unpauseAuction(io) {
  if (!current) throw new Error('No auction is currently active.');
  resumeLot(current.state);
  await persistCurrent();
  io.to('auction-room').emit('auction:state', publicAuctionState());
  return publicAuctionState();
}

/** Admin-triggered skip: resolves the current lot right now instead of
 * waiting out the clock — sold to the current highest bidder if there is
 * one, otherwise unsold — and moves straight to the next lot. */
async function skipLot(io) {
  if (!current) throw new Error('No auction is currently active.');
  const result = skipCurrentLot(current.state);
  if (result) await persistResult(result);
  await persistCurrent();
  if (result) io.to('auction-room').emit('auction:result', result);
  io.to('auction-room').emit('auction:state', publicAuctionState());
  if (current.state.status === 'completed') {
    await finalizeAuctionRecord();
    io.to('auction-room').emit('auction:completed', { soldCount: current.state.soldCount, unsoldCount: current.state.unsoldCount });
...
module.exports = { startAuction, bid, pauseAuction, unpauseAuction, skipLot, autoFillRest, getState, getStageAuctionStatus, resumeAuction };
