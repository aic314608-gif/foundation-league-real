...
-- Owner role: one account above admin that can never be demoted (see
-- routes/admin.js). If no owner exists yet, promote the longest-standing
-- admin account so every league always ends up with exactly one. Safe to
-- run on every boot — a no-op once an owner exists.
DO $$
DECLARE
  first_admin_id INTEGER;
BEGIN
  IF NOT EXISTS (SELECT 1 FROM users WHERE role = 'owner') THEN
    SELECT id INTO first_admin_id FROM users WHERE role = 'admin' ORDER BY created_at ASC, id ASC LIMIT 1;
    IF first_admin_id IS NOT NULL THEN
      UPDATE users SET role = 'owner' WHERE id = first_admin_id;
    END IF;
  END IF;
END $$;
