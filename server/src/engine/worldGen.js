...
// Flat per-tier starting budget: every club in a given stage gets the same
// amount, and it steps down stage-by-stage (Elite -> Rise -> Foundation).
// Indexed by __tierIndex (0 = Elite, 1 = Rise, 2 = Foundation), matching
// the order of STAGE_DEFAULT_NAMES.
const TIER_BUDGETS = [1_000_000_000, 500_000_000, 250_000_000];
const INITIAL_TEAM_BUDGET = TIER_BUDGETS[0]; // kept for any other callers expecting a single default
...
function buildWorld() {
  ...
  const teams = REAL_CLUBS.map((club, i) => ({
    ...
    mentality: 'Balanced',
    budget: TIER_BUDGETS[Math.floor(i / CLUBS_PER_STAGE)] ?? TIER_BUDGETS[TIER_BUDGETS.length - 1],
    youth_level: 1,
...
module.exports = { buildWorld, buildRealPlayer, buildFreeAgent, buildFreeAgentPool, buildCoachPool, INITIAL_TEAM_BUDGET, TIER_BUDGETS };
