...
const { TIER_BUDGETS } = require('../engine/worldGen');
...
// Sets every club's budget to a flat per-tier amount (Elite / Rise /
// Foundation), overwriting whatever they currently have. Use this to apply
// new tier budget numbers to an already-running league without doing a
// full world reset. Does NOT touch anything else (squads, standings, etc).
router.post('/budgets/apply-tier-defaults', async (req, res) => {
  try {
    const { rows: stages } = await pool.query('SELECT id, tier_order FROM stages ORDER BY tier_order');
    let updated = 0;
    for (const stage of stages) {
      const amount = TIER_BUDGETS[stage.tier_order - 1] ?? TIER_BUDGETS[TIER_BUDGETS.length - 1];
      const result = await pool.query('UPDATE teams SET budget = $1 WHERE stage_id = $2', [amount, stage.id]);
      updated += result.rowCount;
    }
    res.json({ ok: true, updated, tierBudgets: TIER_BUDGETS });
  } catch (err) { console.error(err); res.status(500).json({ error: err.message }); }
});
...
router.post('/users/:id/role', async (req, res) => {
  ...
  const { rows: targetRows } = await pool.query('SELECT role FROM users WHERE id = $1', [targetId]);
  if (!targetRows.length) return res.status(404).json({ error: 'User not found.' });
  // The owner is permanent — nobody, including another admin, can demote
  // or reassign them through this endpoint.
  if (targetRows[0].role === 'owner') return res.status(403).json({ error: 'The owner cannot be demoted.' });

  await pool.query('UPDATE users SET role = $1, team_id = $2 WHERE id = $3', [role, role === 'manager' ? teamId : null, targetId]);
  await accountsSvc.writeAudit(null, {
    actorUserId: req.user.id, actorName: req.user.username, action: 'change_role',
    targetType: 'user', targetId, details: { role, teamId: role === 'manager' ? teamId : null },
  });
  res.json({ ok: true });
});
