...
function facilityUpgradeCost(nextLevel) {
  return nextLevel * 10_000_000;
}
...
async function upgradeFacility(teamId, facility) {
  ...
  return withTransaction(async (client) => {
    const { rows } = await client.query(`SELECT * FROM teams WHERE id = $1 FOR UPDATE`, [teamId]);
    const team = rows[0];
    if (!team) throw new Error('Team not found.');
    const current = Number(team[facility]);
    if (current >= 5) throw new Error('Facility is already at the maximum level.');
    const cost = facilityUpgradeCost(current + 1);
