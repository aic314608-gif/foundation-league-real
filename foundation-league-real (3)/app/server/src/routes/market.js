...
const { authRequired, requireTeamAccess } = require('../auth');
...
router.get('/transfer-offers', authRequired, async (req, res) => {
  ...
  if (!teamId && req.user.role !== 'admin') return res.json({ incoming: [], outgoing: [] });
  const scopeClause = req.user.role === 'admin' ? '' : 'WHERE o.from_team_id = $1 OR o.to_team_id = $1';
  const values = req.user.role === 'admin' ? [] : [teamId];
...
router.post('/players/:id/contract', authRequired, async (req, res) => {
  try {
    if (!req.user.teamId && req.user.role !== 'admin') return res.status(400).json({ error: 'You must manage a team to offer contracts.' });
    const teamId = req.body.teamId && req.user.role === 'admin' ? Number(req.body.teamId) : req.user.teamId;
