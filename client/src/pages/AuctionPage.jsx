...
export default function AuctionPage() {
  ...
  const skip = async () => {
    setBusy(true);
    try { await api.post('/auction/skip'); } catch (e) { setError(e.message); }
    setBusy(false);
  };
  const bid = () => {
    const socket = getSocket();
        ...
        <div className="space-y-3">
          ...
          {(state.status === 'active' || state.status === 'paused') && (
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={skip} disabled={busy || !state.currentPlayer} className="px-4 py-2 rounded-md bg-surface-raised border border-line text-chalk-dim hover:text-chalk hover:border-chalk-dim text-sm disabled:opacity-40">
                Skip this player{state.currentBidTeamId ? ' (sell now)' : ''}
              </button>
