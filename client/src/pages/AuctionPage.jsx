...
export default function AuctionPage() {
  ...
  const loadStages = () => {
    if (user?.role === 'admin') api.get('/auction/stages').then((d) => setStages(d.stages)).catch(() => {});
  };
  ...
  const timeLeft = state.deadline ? Math.max(0, Math.round((state.deadline - Date.now()) / 1000)) : 0;
          ...
          <div className="flex flex-wrap gap-2">
            {stages.map((s) => (
              <div key={s.id} className="flex gap-2">
                <button onClick={() => start(s.id, 'coach')} disabled={busy || state.status === 'active'}
                  className="px-4 py-2 rounded-md bg-surface-raised border border-gold text-gold font-semibold text-sm disabled:opacity-40">
                  {s.name} managers{s.coachesCompleted ? ' ✓' : ''}
                </button>
                <button onClick={() => start(s.id, 'player')} disabled={busy || state.status === 'active'}
                  className="px-4 py-2 rounded-md bg-turf text-ink font-semibold text-sm disabled:opacity-40">
                  {s.name} players{s.completed ? ' ✓' : ''}
                </button>
              </div>
            ))}
            {!stages.length && (
              <button onClick={() => start(null, 'player')} disabled={busy || state.status === 'active'} className="px-4 py-2 rounded-md bg-turf text-ink font-semibold text-sm disabled:opacity-40">Start auction (all clubs)</button>
            )}
          </div>
          {state.status === 'active' && (
        ...
        <div className="bg-surface border border-line rounded-lg p-6 space-y-5">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-[11px] uppercase tracking-widest text-chalk-dim">Time left</div>
              <div className={`font-display text-4xl ${timeLeft <= 5 ? 'text-crimson' : 'text-gold'}`}>{timeLeft}s</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between bg-ink-soft rounded-lg px-4 py-3">
            <div>
            ...
            {user?.teamId && (
              <div className="flex items-center gap-2">
                <input value={bidAmount} onChange={(e) => setBidAmount(e.target.value)} placeholder="Your bid"
                  className="w-32 bg-surface border border-line rounded px-2 py-2 text-sm font-mono-tab" />
