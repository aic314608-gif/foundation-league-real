...
export default function TeamPage() {
  ...
  const isManager = user && ((user.role === 'admin' || user.role === 'owner') || (user.role === 'manager' && user.teamId === team.id));
  ...
  return (
    <div className="grid md:grid-cols-2 gap-5">
      <div className="bg-surface border border-line rounded-lg p-5 space-y-3">
        ...
        <FacilityRow label="Youth Academy" level={team.youth_level} tierOrder={team.tier_order} note="Raises youth-intake quantity (1-5/yr) and quality, and speeds player development." onUpgrade={() => upgrade('youth_level')} disabled={!isManager} />
        {team.youth_coach_name && <div className="text-xs text-chalk-dim -mt-2">Youth Coach: {team.youth_coach_name}</div>}
        <FacilityRow label="Medical Centre" level={team.medical_level} tierOrder={team.tier_order} note="Cuts injury recovery time — up to 50% faster at level 5." onUpgrade={() => upgrade('medical_level')} disabled={!isManager} />
...
// Mirrors services/market.js's facilityUpgradeCost so the button can show
// the price before the manager commits — the server remains the source of
// truth and re-derives/validates this same figure on submit.
const TIER_FACILITY_BASE = { 1: 20_000_000, 2: 10_000_000, 3: 5_000_000 };
function previewUpgradeCost(nextLevel, tierOrder) {
  const base = TIER_FACILITY_BASE[tierOrder] ?? TIER_FACILITY_BASE[3];
  return Math.round(base * 2 ** (nextLevel - 2));
}

function FacilityRow({ label, level, tierOrder, note, onUpgrade, disabled }) {
  const cost = level < 5 ? previewUpgradeCost(level + 1, tierOrder) : null;
  return (
    <div>
      <div className="flex items-center justify-between">
      ...
      {!disabled && level < 5 && (
        <button onClick={onUpgrade} className="text-xs text-turf hover:underline">
          Upgrade to level {level + 1} — ${cost.toLocaleString()}
        </button>
      )}
    </div>
  );
