...
function LifecycleTab({ flash }) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-5">
      ...
      <div className="bg-surface border border-line rounded-lg p-5 space-y-3">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-chalk-dim">Finances</h3>
        <Row label="Apply tier budgets" note="Sets every club's budget to a flat amount for its division — Elite $1B, Rise $400M, Foundation $150M — overwriting current budgets. Squads, results, and everything else are untouched.">
          <DangerButton label="Apply" onClick={() => call(() => api.post('/admin/budgets/apply-tier-defaults', {}), (r) => `Budgets updated for ${r.updated} clubs.`)} confirmText="Overwrite every club's current budget?" />
        </Row>
      </div>
          ...
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-b border-line/50 last:border-0">
                ...
                <td className="px-3 py-2 capitalize">{u.role}{u.role === 'owner' && <span className="ml-1.5 text-[10px] uppercase tracking-wide text-gold/70">(permanent)</span>}</td>
                <td className="px-3 py-2 text-chalk-dim">{u.team_name || '—'}</td>
                <td className="px-3 py-2">
                  {u.role === 'owner' ? (
                    <span className="text-xs text-chalk-dim italic">Owner cannot be changed</span>
                  ) : (
